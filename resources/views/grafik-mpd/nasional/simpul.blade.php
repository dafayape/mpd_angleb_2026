...
@section('content')
            ...
            <ul class="nav nav-pills" id="simpul-tabs" role="tablist">
                @foreach (['DARAT', 'LAUT', 'UDARA', 'KERETA'] as $index => $tab)
                    <li class="nav-item" role="presentation">
                        <button class="nav-link {{ $index === 0 ? 'active' : '' }}" id="tab-{{ strtolower($tab) }}"
                            data-bs-toggle="pill" data-bs-target="#content-{{ strtolower($tab) }}" type="button"
                            role="tab">
                            SIMPUL {{ $tab }}
                        </button>
                    </li>
    ...
    <div class="tab-content">
        @foreach (['DARAT', 'LAUT', 'UDARA', 'KERETA'] as $index => $tab)
            <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="content-{{ strtolower($tab) }}"
                role="tabpanel">
                            ...
                            <div class="card-body">
                                ...
                                @if (isset($section['subtitle']))
                                    <p class="text-muted mb-4"
                                        style="margin-top: -15px; margin-left: 15px; font-size: 0.9rem;">
                                        {{ $section['subtitle'] }}</p>
                                @endif
                                        ...
                                        <div class="col-12 mb-4">
                                            <div class="row">
                                                <div class="col-xl-9 col-lg-8">
                                                    <h6 class="card-title text-center mb-3">Pergerakan Harian
                                                        {{ $chart['label'] }} (Real & Forecast)</h6>
                                                    <div id="chart-daily-{{ $tab }}-{{ $sIdx }}-{{ $cIdx }}"
                                                        class="chart-container-daily"></div>
                                                </div>
                                                <div class="col-xl-3 col-lg-4">
                                                    <div class="kpi-card">
                                                        ...
                                                        @php
                                                            ...
                                                            if (str_contains($lowerLabel, 'bus')) {
                                                                $icon = 'mdi-bus';
                                                            } elseif (str_contains($lowerLabel, 'penyeberangan')) {
                                                                $icon = 'mdi-ferry';
                                                            } elseif (str_contains($lowerLabel, 'laut')) {
                                                                $icon = 'mdi-anchor';
                                                            } elseif (str_contains($lowerLabel, 'udara')) {
                                                                $icon = 'mdi-airplane';
                                                            } elseif (str_contains($lowerLabel, 'ka')) {
                                                                $icon = 'mdi-train';
                                                            }
                                                        @endphp
                                                        <div class="kpi-icon"><i
                                                                class="mdi {{ $icon }} text-primary"></i></div>
                                                        <h6 class="text-uppercase text-muted mb-3">{{ $chart['label'] }}
                                                        </h6>
                                                        <div class="mb-2">
                                                            <span class="text-real fw-bold">Total Real:</span>
                                                            <div class="text-real h2 fw-bold">
                                                                {{ number_format($chart['total_real'], 0, ',', '.') }}
                                                            </div>
                                                        </div>
                                                        <div>
                                                            <span class="text-forecast fw-bold">Total Forecast:</span>
                                                            <div class="text-forecast h4 fw-bold">
                                ...
                                @if (!empty($section['top_origin']))
                                    <div class="row">
                                        <div class="col-xl-6">
                                            <h6 class="card-title text-center mb-3">10 BESAR
                                                {{ strtoupper($section['title']) }} - ASAL</h6>
                                            <div class="text-center text-muted small mb-2">{{ $section['subtitle'] ?? '' }}
                                            </div>
                                            <div id="chart-top-origin-{{ $tab }}-{{ $sIdx }}"
                                                class="chart-container-top"></div>
                                        </div>
                                        <div class="col-xl-6">
                                            <h6 class="card-title text-center mb-3">10 BESAR
                                                {{ strtoupper($section['title']) }} - TUJUAN</h6>
                                            <div class="text-center text-muted small mb-2">{{ $section['subtitle'] ?? '' }}
                                            </div>
                                            <div id="chart-top-dest-{{ $tab }}-{{ $sIdx }}"
                                                class="chart-container-top"></div>
                                        </div>
                                    </div>
                                @endif
                                ...
                                @if (!empty($section['top_od']))
                                    <div class="row mt-4">
                                        <div class="col-12">
                                            <h6 class="card-title text-center mb-3">O-D SIMPUL
                                                {{ strtoupper($section['title']) }}</h6>
                                            <div class="text-center text-muted small mb-2">{{ $section['subtitle'] ?? '' }}
                                            </div>
                                            <div id="chart-top-od-{{ $tab }}-{{ $sIdx }}"
                                                class="chart-container-top" style="height: 500px"></div>
                                        </div>
                                    </div>
                                @endif
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const colors = ['#29b6f6', '#5c6bc0', '#00e676', '#ff7043', '#8d6e63', '#ab47bc', '#ef5350', '#ffca28',
                '#26c6da', '#78909c'
            ];
                ...
                tData.sections.forEach((section, sIdx) => {
                    ...
                    section.daily_charts.forEach((chartData, cIdx) => {
                        Highcharts.chart(`chart-daily-${tab}-${sIdx}-${cIdx}`, {
                            ...
                            series: [{
                                name: 'Real',
                                data: chartData.series_real
                            }, {
                                name: 'Forecast',
                                data: chartData.series_forecast
                            }],
                            credits: {
                                enabled: false
                            }
                    ...
                    if (section.top_origin && section.top_origin.length > 0) {
                        renderBarChart(`chart-top-origin-${tab}-${sIdx}`, section.top_origin,
                            colors);
                    }
