            ...
            <div class="col-md-3">
                <div class="card border-start border-4 border-primary h-100">
                    <div class="card-body py-3">
                        <h6 class="text-muted small mb-1">Pergerakan (Real)</h6>
                        <h4 class="mb-0 fw-bold text-primary">
                            {{ number_format($charts['summary']['real'] ?? 0, 0, ',', '.') }}</h4>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card border-start border-4 border-warning h-100">
                    <div class="card-body py-3">
                        <h6 class="text-muted small mb-1">Pergerakan (Forecast)</h6>
                        <h4 class="mb-0 fw-bold text-warning">
                            {{ number_format($charts['summary']['forecast'] ?? 0, 0, ',', '.') }}</h4>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card border-start border-4 border-info h-100">
                    <div class="card-body py-3">
                        <h6 class="text-muted small mb-1">Unique Subscriber (Real)</h6>
                        <h4 class="mb-0 fw-bold text-info">
                            {{ number_format($charts['summary']['people'] ?? 0, 0, ',', '.') }}</h4>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card border-start border-4 border-secondary h-100">
                    <div class="card-body py-3">
                        <h6 class="text-muted small mb-1">Unique Subscriber (Forecast)</h6>
                        <h4 class="mb-0 fw-bold text-secondary">
                            {{ number_format($charts['summary']['people_forecast'] ?? 0, 0, ',', '.') }}</h4>
                    </div>
                </div>
            </div>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            try {
                ...
                Highcharts.chart('chart-orang-trend', Highcharts.merge(commonOptions, {
                    series: data.series_trend_orang || data.series_trend
                }));
                ...
                Highcharts.chart('chart-orang-opsel', Highcharts.merge(commonOptions, {
                    series: data.series_opsel_orang || data.series_opsel
                }));
