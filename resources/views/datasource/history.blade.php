...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">History File Upload</h4>
                            ...
                            <div class="flex-grow-1">
                                ...
                                <h6 class="mb-0" style="font-size: 12px;">
                                    @foreach ($summary['by_opsel'] as $opsel => $count)
                                        <span class="badge bg-primary bg-soft text-primary">{{ $opsel }}: {{ number_format($count) }}</span>
                                    @endforeach
                                </h6>
                            </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-12">
                            <form action="{{ route('datasource.history') }}" method="GET" class="row g-3 align-items-end">
                                <div class="col-md-3">
                                    <label for="opsel" class="form-label" style="font-size: 11px; text-transform: uppercase;">Opsel</label>
                                ...
                                <div class="col-md-3">
                                    <label for="kategori" class="form-label" style="font-size: 11px; text-transform: uppercase;">Tipe</label>
                                    <select class="form-select" id="kategori" name="kategori" style="font-size: 12px;">
                                        <option value="">Semua Tipe</option>
                                        <option value="REAL" {{ request('kategori') == 'REAL' ? 'selected' : '' }}>REAL</option>
                                        <option value="FORECAST" {{ request('kategori') == 'FORECAST' ? 'selected' : '' }}>FORECAST</option>
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label" style="font-size: 11px; text-transform: uppercase;">Rentang Tanggal Data</label>
                                    <div class="input-group input-group-sm">
                                        <input type="date" class="form-control" name="start_date" value="{{ request('start_date') }}" style="font-size: 12px;">
                                        <span class="input-group-text">s/d</span>
                                        <input type="date" class="form-control" name="end_date" value="{{ request('end_date') }}" style="font-size: 12px;">
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="d-flex gap-2">
                                        <button type="submit" class="btn btn-primary btn-sm w-100"><i class="bx bx-filter-alt me-1"></i> Filter</button>
                                        <a href="{{ route('datasource.history') }}" class="btn btn-secondary btn-sm w-100"><i class="bx bx-reset me-1"></i> Reset</a>
                                    </div>
                                </div>
                            </form>
                        </div>
                            ...
                            <tbody style="font-size: 12px;">
                                @forelse($histories as $history)
                                    <tr>
                                        ...
                                        <td class="fw-bold">{{ $history->tanggal_data ? \Carbon\Carbon::parse($history->tanggal_data)->format('Y-m-d') : '-' }}</td>
                                        <td><span class="badge bg-primary bg-soft text-primary font-size-11">{{ $history->opsel ?? '-' }}</span></td>
                                        <td>{{ $history->kategori ?? '-' }}</td>
                                        <td>{{ number_format($history->total_rows ?: $history->processed_rows) }} Rows</td>
                                        <td><span class="text-dark fw-medium">{{ $history->original_filename ?? $history->filename }}</span></td>
                                        <td>{{ $history->created_at->format('Y-m-d H:i:s') }}</td>
                                        <td>
                                            @if ($history->status == 'completed')
                                                <span class="badge badge-pill badge-soft-success font-size-11">Completed</span>
                                            @elseif ($history->status == 'processing')
                                                <span class="badge badge-pill badge-soft-warning font-size-11">Processing</span>
                                            @elseif ($history->status == 'failed')
                                                <span class="badge badge-pill badge-soft-danger font-size-11">Failed</span>
                                            @else
                                                <span class="badge badge-pill badge-soft-secondary font-size-11">{{ ucfirst($history->status) }}</span>
                                            @endif
                                            @if ($history->status == 'failed' && $history->error_message)
                                                <div class="text-danger mt-1" style="font-size: 10px;">{{ Str::limit($history->error_message, 20) }}</div>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="d-flex gap-2">
                                                <a href="{{ route('datasource.raw-data', ['tanggal' => $history->tanggal_data, 'opsel' => $history->opsel]) }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" title="View Raw Data">
                                                    <i class="bx bx-show-alt"></i>
                                                </a>
                                                <button type="button" class="btn btn-sm btn-danger" onclick="confirmDelete({{ $history->id }}, {{ $history->total_rows ?: $history->processed_rows }})" data-bs-toggle="tooltip" title="Hapus Data">
    ...
    <div class="modal fade" id="deleteProgressModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="progress mb-3">
                        <div id="deleteProgressBar" class="progress-bar progress-bar-striped progress-bar-animated bg-danger" role="progressbar" style="width: 0%">0%</div>
                    </div>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'progress') {
                        ...
                        let percentage = totalRows > 0 ? Math.min(Math.round((deletedSoFar / totalRows) * 100), 99) : 50;
                        progressBar.style.width = percentage + '%';
                        progressBar.innerHTML = percentage + '%';
                        statusText.innerText = 'Terhapus: ' + new Intl.NumberFormat('id-ID').format(deletedSoFar) + ' baris...';
                        deleteChunk();
