    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">Log Aktivitas</h4>
                ...
                <div class="card-body">
                    <div class="d-flex flex-wrap align-items-center justify-content-between mb-3">
                        ...
                        <div class="d-flex gap-2">
                            <a href="{{ route('log-aktivitas.export', request()->query()) }}" class="btn btn-sm btn-outline-secondary">
                    ...
                    <form id="filterForm" method="GET" action="{{ route('log-aktivitas') }}" class="mb-3">
                        <div class="row g-2 align-items-end">
                            <div class="col-md-3">
                                <input type="text" name="search" class="form-control form-control-sm" placeholder="Cari user / aktivitas..." value="{{ request('search') }}">
                            </div>
                            <div class="col-md-2">
                            ...
                            <div class="col-md-1">
                                <div class="d-flex gap-1">
                                    ...
                                    @if(request()->hasAny(['search','status','user_id','start_date','end_date']))
                                        <a href="{{ route('log-aktivitas') }}" class="btn btn-sm btn-outline-secondary"><i class="bx bx-x"></i></a>
                                    @endif
                                </div>
                            </div>
                        </div>
                            ...
                            <tbody>
                                @forelse($logs as $log)
                                    <tr>
                                        ...
                                        <td>
                                            <small class="text-muted"><i class="bx bx-time"></i> {{ $log->created_at->format('d M Y, H:i') }}</small>
                                        </td>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <span class="avatar-xs me-2">
                                                    <span class="avatar-title rounded-circle bg-primary bg-soft text-primary font-size-12">{{ strtoupper(substr($log->user->name ?? 'S', 0, 1)) }}</span>
                                                </span>
                                                {{ \Illuminate\Support\Str::limit($log->user->name ?? 'System', 18) }}
                                            </div>
                                        </td>
                                        <td><span class="badge bg-primary bg-soft text-primary">{{ $log->action }}</span></td>
                                        <td class="text-truncate" style="max-width:200px" title="{{ $log->subject }}">{{ $log->subject ?? '-' }}</td>
                                        <td class="text-center">
                                            @if($log->status === 'Success')
                                                <span class="badge bg-success bg-soft text-success"><i class="bx bx-check-circle"></i> Success</span>
                                            @else
                                                <span class="badge bg-danger bg-soft text-danger"><i class="bx bx-x-circle"></i> {{ $log->status }}</span>
                                            @endif
                                        </td>
                                        <td class="text-center">
                    ...
                    @if($logs->hasPages())
                        <div class="d-flex justify-content-between align-items-center mt-3">
                            <small class="text-muted">Menampilkan {{ $logs->firstItem() }}–{{ $logs->lastItem() }} dari {{ $logs->total() }}</small>
                            {{ $logs->links() }}
                        </div>
                    @endif
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <table class="table table-bordered table-sm mb-0">
                        <tr><th width="35%">Waktu</th><td id="modal-time"></td></tr>
                        <tr><th>User</th><td id="modal-user"></td></tr>
                        <tr><th>Aktivitas</th><td id="modal-action"></td></tr>
                        <tr><th>Target Objek</th><td id="modal-subject"></td></tr>
                        <tr><th>Status</th><td id="modal-status"></td></tr>
                        <tr><th>IP Address</th><td id="modal-ip"></td></tr>
                        <tr><th>User Agent</th><td id="modal-agent" style="word-break:break-all;font-size:.85rem"></td></tr>
                        <tr><th>Deskripsi</th><td id="modal-description"></td></tr>
                    </table>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            modal.addEventListener('show.bs.modal', function (e) {
                ...
                ['time','user','action','subject','status','ip','agent','description'].forEach(function (key) {
                    document.getElementById('modal-' + key).textContent = btn.getAttribute('data-' + key) || '-';
                });
            });
        });
    </script>
